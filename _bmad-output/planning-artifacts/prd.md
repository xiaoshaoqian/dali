...
- **沉浸式呈现 (Immersive Hero Mode @ L5)**：
  - 生成结果页采用 **"沉浸式主角布局"** (Immersive Hero Layout)
  - Hero 图片占据首屏 52%，顶部使用品牌紫色调透明渐变
  - 底部 Content Sheet 以 32px 圆角叠加，形成杂志封面感
  - Loading 页与 Result 页布局完全一致，实现无感知过渡
- **等待视觉优化 (Progressive Visual Disclosure)**：
  - **模糊显影图 (Blur-to-Clear)**：图片从模糊 (blur 20px) 渐变清晰，跟随生成进度
  - **思维流文字 (Streaming Text)**：AI 思考过程以逐字模糊显现效果展示
